...
Standalone image (own title bar / frame / status bar) -- sized independently
of the info card so it can use full ASCII detail without needing to match
another panel's height.
"""
...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "ghostmikz-ascii.svg")
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">ghostmikz@github: ~$ ./portrait.sh</text>')
