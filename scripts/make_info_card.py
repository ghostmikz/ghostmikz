...
the ASCII portrait: colored key/value rows for role, education, stack, and
highlights. Contact links live as a small badge row in the README instead of
here (badges are clickable; SVG text rows aren't when embedded via <img>).
...
Standalone image, sized to fit its own content -- not stretched or squeezed
to match the portrait's height.
"""
import html
import os
...
# ---- first pass: measure content height so canvas fits tightly -----------
_y = TITLEBAR_H + 30
for row in ROWS:
    _y += LINE_H * 0.5 if row[0] == "gap" else LINE_H
H = round(_y + 20)
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">ghostmikz@github: ~$ neofetch</text>')
